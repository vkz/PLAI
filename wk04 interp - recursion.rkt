...
(define-type ExprS
  ...
  [recS (name symbol?) (val ExprS?) (body ExprS?)]
...
(define num-lr
  (λ (op l r)
    (cond
      [(and (numV? l) (numV? r)) 
       (cond 
         [(equal? op =) (boolV (op (numV-n l) (numV-n r)))]      
      ...
      [multC (l r) (type-case Result (interp l env sto)
                     [v*s (v-l s-l) 
                          (type-case Result (interp r env s-l)
                            [v*s (v-r s-r) 
                                (v*s (num* v-l v-r) s-r)])])]
      [=C (l r) (type-case Result (interp l env sto)
                  [v*s (v-l s-l) 
                       (type-case Result (interp r env s-l)
                         [v*s (v-r s-r) 
                              (v*s (num= v-l v-r) s-r)])])]
...
(test (get-value
       (interpret
        (desugar 
         (recS 'fact 
               (lamS 'n (ifS (=S (idS 'n) (numS 0))
                             (numS 1)
                             (multS (idS 'n) (appS (idS 'fact) (bminusS (idS 'n) (numS 1))))))
               (appS (idS 'fact) (numS 3))))))
      6)
